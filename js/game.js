...
- [추가] var sky;  
  [이유] Sky 전역 변수를 추가하여 "can't find variable: sky" 오류 방지

- [변경] shading: THREE.FlatShading → flatShading: true  
  [이유] Three.js 최신 버전에서 shading 속성이 제거되어 flatShading 사용

- [변경] shadowLight.shadow.mapSize.width  = 2048;  
        shadowLight.shadow.mapSize.height = 2048;  →  shadowLight.shadow.mapSize.set(2048, 2048);  
  [이유] 최신 Three.js에서 shadow mapSize 설정 방식이 변경됨

- [변경] var Sky, var Sea, var Cloud = function() {...}  → function Sky() {...}, function Sea() {...}, function Cloud() {...}  
  [이유] 함수 선언 방식을 변경하여 호이스팅 문제 해결 및 최신 문법 적용
...
Sea.prototype.moveWaves = function () {
  ...
  for (let i = 0; i < l; i++) {
    ...
    verts[idx] = vprops.x + Math.cos(vprops.ang) * vprops.amp; // x
    verts[idx + 1] = vprops.y + Math.sin(vprops.ang) * vprops.amp; // y
    verts[idx + 2] = vprops.z; // z는 그대로 유지
